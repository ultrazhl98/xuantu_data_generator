...
def _make_video_row_col(slot: SlotInfo, rel_col: int,
...
def generate_video_sequential_samples(
    ...
    # 预计算同行内相对列号（只在传入的视频 slots 中计数，用于"第X排第Y个"）
    ...
    for slot in slots:
        ...
        if slot.row > 0 or slot.col > 0:
            buckets["row_col"].append(_make_video_row_col(slot, slot_rel_col[id(slot)], image_path, app, rng))
