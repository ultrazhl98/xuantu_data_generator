...
def _make_video_row_col(slot: SlotInfo, rel_row: int, rel_col: int,
...
def generate_video_sequential_samples(
    ...
    # 预计算同类型相对行列号（只在传入的视频 slots 中计算，避免图片/拍照位干扰）
    ...
    for slot in slots:
        ...
        if rel_r > 1 or rel_c > 1:
            buckets["row_col"].append(_make_video_row_col(slot, rel_r, rel_c, image_path, app, rng))
